...
export default function KioskScreen() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (opType === 'PICKUP') {
                if (!form.reference_number) { setLoading(false); return Alert.alert('Missing Info', 'Booking Reference is required.'); }
                const { data: release, error } = await supabase.from('booking_releases').select('customer, active')
                    .ilike('reference', form.reference_number.trim()).eq('active', true).single();
                if (error || !release) { setLoading(false); return Alert.alert('Invalid Reference', 'This Booking Reference does not exist or is not active.'); }
                finalCustomerId = release.customer;
            }
            if (opType === 'DROP OFF') {
                if (!form.container_prefix || !form.container_number) { setLoading(false); return Alert.alert('Missing Info', 'Container prefix and number are required.'); }
                const { data: container, error } = await supabase.from('containers').select('customer')
                    .eq('prefix', form.container_prefix.toUpperCase().trim()).eq('number', form.container_number.trim()).single();
                if (error || !container) { setLoading(false); return Alert.alert('Container Not Found', 'This container is not registered. Please check with your operator.'); }
                finalCustomerId = container.customer;
            }
            const { error } = await supabase.from('kiosk_submissions').insert([{
